...
# ===================== CONFIG =====================
BOT_TOKEN = "8972509344:AAEpPyjnaCPOcrq1JezQ7xdmIGY-q8sJwuM"
ADMIN_ID = 8690101844
...
# ===================== DATABASE =====================
def init_db():
    conn = sqlite3.connect("bot_data.txt")
    c = conn.cursor()
    c.execute("""
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ---- নির্দিষ্ট লিংক কিনুন ----
        ...
        # লিংক এখন ক্লিকযোগ্য বাটন হিসেবে পাঠানো হবে
        keyboard = [
    ...
    # ==================== অ্যাডমিন ====================
        ...
        keyboard = [
            ...
            [InlineKeyboardButton("📢 প্রমোশন", callback_data="admin_promotion")],  # নতুন বাটন
            [InlineKeyboardButton("🏠 হোম", callback_data="home")]
        ]
        await query.edit_message_text(
    ...
    # -------------------------- নতুন প্রমোশন সেকশন --------------------------
    elif data == "admin_promotion":
        if user.id != ADMIN_ID:
            return
        # প্রমোশন স্টেপ শুরু
        context.user_data["promo_step"] = "link"
        keyboard = [[InlineKeyboardButton("❌ বাতিল", callback_data="cancel_promotion")]]
        await query.edit_message_text(
            "📢 *নতুন প্রমোশন তৈরি*\n\n"
            "প্রথমে চ্যানেল/গ্রুপের *লিংক* দিন (https://t.me/...):",
            reply_markup=InlineKeyboardMarkup(keyboard),
            parse_mode="Markdown"
        )

    elif data == "cancel_promotion":
        if user.id != ADMIN_ID:
            return
        # প্রমোশন ডাটা ক্লিয়ার
        context.user_data.pop("promo_step", None)
        context.user_data.pop("promo_link", None)
        context.user_data.pop("promo_channel_id", None)
        context.user_data.pop("promo_coins", None)
        await query.edit_message_text(
            "❌ প্রমোশন তৈরি বাতিল করা হয়েছে।",
            reply_markup=main_menu_keyboard(user.id)
        )
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ====== প্রমোশন স্টেপ হ্যান্ডলিং (শুধু অ্যাডমিন) ======
    if user.id == ADMIN_ID and context.user_data.get("promo_step"):
        step = context.user_data["promo_step"]

        if step == "link":
            if not (text.startswith("https://t.me/") or text.startswith("http://t.me/") or text.startswith("t.me/")):
                await update.message.reply_text("❌ অনুগ্রহ করে সঠিক টেলিগ্রাম লিংক দিন (https://t.me/...)")
                return
            context.user_data["promo_link"] = text.strip()
            context.user_data["promo_step"] = "channel_id"
            keyboard = [[InlineKeyboardButton("❌ বাতিল", callback_data="cancel_promotion")]]
            await update.message.reply_text(
                "✅ লিংক সংরক্ষিত।\n\nএখন চ্যানেল/গ্রুপের *আইডি* দিন (যেমন -100XXXXXXXXX):",
                reply_markup=InlineKeyboardMarkup(keyboard),
                parse_mode="Markdown"
            )

        elif step == "channel_id":
            # সিম্পল ভ্যালিডেশন: ইন্টিজার হতে হবে, নেগেটিভ হলে ভালো
            try:
                int(text.strip())
            except ValueError:
                await update.message.reply_text("❌ আইডি অবশ্যই একটি সংখ্যা হতে হবে। আবার চেষ্টা করুন।")
                return
            context.user_data["promo_channel_id"] = text.strip()
            context.user_data["promo_step"] = "coins"
            keyboard = [[InlineKeyboardButton("❌ বাতিল", callback_data="cancel_promotion")]]
            await update.message.reply_text(
                "✅ আইডি সংরক্ষিত।\n\nএখন ইউজাররা জয়েন করলে কত *কয়েন* পাবে তা লিখুন (শুধু সংখ্যা):",
                reply_markup=InlineKeyboardMarkup(keyboard),
                parse_mode="Markdown"
            )
            ...
            try:
                ...
                await update.message.reply_text("❌ অনুগ্রহ করে একটি ধনাত্মক পূর্ণসংখ্যা লিখুন।")
                return

            # সব তথ্য সংগ্রহ শেষ
            promo_link = context.user_data["promo_link"]
            promo_id = context.user_data["promo_channel_id"]
            promo_coins = coins
            ...
            promo_msg = (
                f"📢 *নতুন প্রমোশন!* 🎁\n\n"
                f"🔗 চ্যানেল/গ্রুপ লিংক: {promo_link}\n"
                f"🆔 চ্যানেল আইডি: `{promo_id}`\n\n"
                f"💰 এই চ্যানেলে জয়েন করলে পাবেন *{promo_coins} কয়েন*!\n"
                f"সবাই জয়েন করুন এবং কয়েন উপার্জন করুন।"
            )
            ...
            for uid, _, _ in users:
                try:
                    await context.bot.send_message(uid, promo_msg, parse_mode="Markdown")
                    sent += 1
                except:
                    failed += 1
            ...
            await update.message.reply_text(
                f"✅ *প্রমোশন মেসেজ পাঠানো সম্পন্ন!*\n\n"
                f"📤 সফল: {sent} জন\n"
                f"❌ ব্যর্থ: {failed} জন",
                reply_markup=InlineKeyboardMarkup(keyboard),
                parse_mode="Markdown"
            )
            return  # অন্যান্য হ্যান্ডলিং এড়াতে

    # ====== বিদ্যমান adding_link এবং broadcasting হ্যান্ডলিং ======
